...
export let grammar: { [key: string]: string[] } = {};
export let grammarKeys: string[] = [];
let tags: string[] = [];

export let loadedFiles: string[] = [];

export function reloadTraceryData() {
    grammar = {};
    loadedFiles = [];
    grammarKeys = [];
    tags = [];
...
function loadGrammar(filename: string) {
    ...
    Object.keys(grammarTemp).forEach((key) => {
        if (grammar[key] === undefined) {
            ...
            grammar[key] = grammar[key].concat(grammarTemp[key]);
        }
    });
    ...
    grammarKeys.push(
        ...Object.keys(grammar).filter((x) => {
            if (x.includes("!")) {
                return false;
            }
            if (x.includes("origin")) {
                return false;
            }
            if (x.includes("intro_question")) {
                return false;
            }
            if (x.includes("close_question")) {
                return false;
            }
    ...
    keys.forEach((key) => {
        ...
        grammar[key].forEach((value: string, index: number) => {
            ...
            if (!key.startsWith("_") && dupIndex < index && dupIndex >= 0) {
                console.warn(
                    `Duplicate value: "${key}"": "${value}" (${index})`
                );
            }

            if (value.includes(" a #") || value.toLowerCase().startsWith('a #')) {
                console.warn(
                    `Use modified instead for a/an: ${key}:${index} (${value.substr(
                        0,
                        20
                    )})`
                );
